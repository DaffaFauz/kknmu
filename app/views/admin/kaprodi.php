...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card">
        <div class="card-header border-bottom d-flex justify-content-between align-items-center my-0">
            <h5 class="card-title mb-0">Data Kaprodi</h5>
            <button type="button" class="btn btn-primary ps-2" data-bs-toggle="modal" data-bs-target="#tambahKaprodi">
                <i class="ti tabler-plus me-1"></i> Tambah Kaprodi
            </button>
        </div>
                ...
                <tbody>
                    ...
                    foreach ($data['kaprodi'] as $row):
                        ...
                        <tr>
                            ...
                            <td>
                                <form class="d-inline"
                                    action="<?= BASE_URL ?>/Kaprodi/delete/<?= htmlspecialchars($row['id_user']) ?>"
                                    method="post">
                                    <button type="submit" class="btn btn-danger"
                                        onClick="return confirm('Yakin ingin melepaskan jabatan dosen ini dari kaprodi?')"><i
...
<div class="modal fade" id="tambahKaprodi" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-md modal-simple modal-edit-user">
        <div class="modal-content">
            <div class="modal-body">
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                <div class="text-center mb-6">
                    <h4 class="mb-2">Tambah Kaprodi</h4>
                </div>
                <form id="tambahKaprodiForm" class="row g-6" action="<?= BASE_URL ?>/Kaprodi/create" method="post">
                    <div class="col-12">
                        <label class="form-label" for="modalTambahKaprodi">Dosen</label>
                        <select name="id_kaprodi" id="modalTambahKaprodi" class="form-select" required>
                            <option value="">Pilih Dosen</option>
                            <?php foreach ($data['notKaprodi'] as $row): ?>
